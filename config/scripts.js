...
var cmd = {
	...
	jsbeautify: windowsCheck('node_modules/.bin/js-beautify')
};
...
exports.startDev = function startDev() {
	...
	return spawn(`${cmd.webpackDevServer} --progress --inline --env dev`, {
		shell: true,
		stdio: 'inherit'
	});
};
...
exports.startProd = function startProd() {
	...
	return spawn(`${ngc} && ${cmd.webpack} --progress --hide-modules true --env prod && node -e "require('./config/helpers').cleanup('client')" && node dist`, {
		shell: true,
		stdio: 'inherit'
	});
};
...
exports.beautify = function beautify() {
	console.log(chalk.bold.magenta('\n\tPlease Wait ... Beautifying files accourding to our .jsbeautifyrc\n\n'));

	return spawn(`${cmd.jsbeautify} --config .jsbeautifyrc -r -f \`find . \\( -name '*.json' -o -name '*.js' -o -name '*.ts' -o -name '*.html' \\) -not -path './node_modules/*'\``, {
		shell: true,
		stdio: 'inherit'
	});
};