...
import FeedingEvents from './feeding.events';
...
function register(socket) {
	...
	for (let i = 0, eventsLength = events.length; i < eventsLength; i++) {
		let event = events[i];
		let listener = createListener('Feeding:' + event, socket);

		// the Emiter will listen for changes in the model
		FeedingEvents.on(event, listener);
...
function removeListener(event, listener) {
	return function () {
		...
		FeedingEvents.removeListener(event, listener);
	};
}

// export the es6 way 
export { register as feedingRegister };