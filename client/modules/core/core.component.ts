...
export class CoreComponent implements AfterViewInit {
	...
	ngAfterViewInit() {
		...
		this.formTimeline = new TimelineMax({
			paused: true
		});
		this.formTimeline
			.to(this.formToast.nativeElement.children[0], 0, {
				ease: Power0.easeNone,
				display: 'block'
			})
			.fromTo(this.formToast.nativeElement.children[0], 1, {
				y: -500
			}, {
				y: 0
			});

		this.formTimeline2 = new TimelineMax({
			paused: true
		});
		this.formTimeline2
			.to(this.formToast.nativeElement.children[1], 0, {
				ease: Power0.easeNone,
				display: 'block'
			})
			.fromTo(this.formToast.nativeElement.children[1], 1, {
				y: -500
			}, {
				y: 0
			});
		...
		this.errorTimeline = new TimelineMax({
			paused: true
		});
		this.errorTimeline
			.to(this.errorToast.nativeElement, 0, {
				display: 'block',
				y: 400
			})
			.to(this.errorToast.nativeElement, 1, {
				y: 0
			})
			.to(this.errorToast.nativeElement, 1, {
				y: 400,
